...
export function AnalyticsTrendChart({ metric, points, resolution }) {
  ...
  const chart = useMemo(() => {
    ...
    return {
      ...
      selected: coordinates[selectedIndex] || coordinates[coordinates.length - 1] || null,
      width,
  ...
  if (!chart.coordinates.length) {
    return <p className="empty-state">No hay puntos suficientes para graficar este rango.</p>;
  }
  ...
  return (
    <section className="analytics-card analytics-main-chart" aria-labelledby="analytics-trend-title">
      ...
      <div className="chart-wrap">
        <svg className="trend-svg analytics-svg" viewBox={`0 0 ${chart.width} ${chart.height}`} role="img">
          ...
          <g className="chart-axis">
            {chart.coordinates.map((point, index) => {
              const isVisible = index === 0 || index === chart.coordinates.length - 1 || index % 4 === 0;
              return isVisible ? (
